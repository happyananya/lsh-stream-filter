"""
Run Jaccard Aggregator Only
===========================
Runs the BucketOccupancy policy using the 'jaccard' aggregator and appends
the results to the existing Phase 4 HeavyDuplication CSV.
"""
import os
import sys
import numpy as np
...
def run_jaccard_sweep():
    ...
    print(f"\n{'#'*70}")
    print(f"Running Jaccard Sweep on: {stream_name}")
    print(f"{'#'*70}")
    ...
    for frac in budget_fractions:
        ...
        policy_name = 'BucketOccupancy (Jaccard)'
        
        if (policy_name, frac) in completed_runs:
            print(f"  Skipping {policy_name} (already computed for frac {frac})")
            continue
            
        policy = BucketOccupancyRetention(dim=dim, L=L_opt, K=K_opt, capacity=B, aggregator='jaccard')
    ...
    print(f"\nCompleted Jaccard sweep. Results appended to {csv_path}.")
