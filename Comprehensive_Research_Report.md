# Comprehensive Research Report: LSH Bucket-Occupancy Retention for Bounded Streaming Memory

## 1. Goal and Introduction

Modern language models, AI agents, and streaming analytics systems face a fundamental constraint: context windows and fast-memory buffers are strictly bounded in size. When ingesting a continuous, unbounded stream of text embeddings, a retention policy must decide what to keep and what to evict to maximize the semantic diversity and recall of the retained memory $M$ under a fixed budget $|M| \le B$.

The primary goal of this research project is to develop and evaluate a highly efficient streaming retention policy that maximizes the semantic coverage (diversity) of a retained memory buffer without the $O(\|M\|)$ overhead of exact nearest-neighbor deduplication.
...
**Analysis:** Sweeping the hash length $K$ from 6 to 14 reveals stable recall performance across all values. $K=10$ provides a good balance between hash quality and compute cost for 384-dimensional embeddings.

### Distribution Drift
![Drift Response](results/phase2_characterization/plots/exp5_drift_response.png)
**Analysis:** Under sequential topic drift, the retention rate decays monotonically as the memory cap is reached. The policy performs comparably to Reservoir sampling in this regime, suggesting the bucket-novelty signal is most valuable when redundancy is the dominant structure rather than drift.
...
**Key Finding (Negative Result):** Under topic drift without heavy duplication, BucketOccupancy performs **slightly worse** than both FIFO and Reservoir. FIFO leads because recency correlates with relevance when topics shift sequentially. This is an important negative result: **bucket-occupancy retention provides no advantage when distribution drift is the only dynamic at play.**

---

## 6. Phase 5: LoCoMo LLM-QA Benchmark
...
### 6.1 QA Accuracy Results (Mean per-conversation)
...
![Phase 5 Accuracy Plot](results/phase5_locomo/phase5_accuracy_plot.png)
...
## 7. Phase 6: Johnson-Lindenstrauss (JL) Projection Tradeoff

The goal of this experiment was to test whether **Johnson-Lindenstrauss (JL) projection** can compound the memory advantage of `BucketOccupancy`. By projecting 384D embeddings down to $d=64$, we can store exactly **6 times as many items** for the same fixed memory footprint (in bytes).

### 7.1 Experimental Results (Recall@10)

| Memory Footprint | Policy | Dimension ($d$) | Items Capacity | Recall@10 |
| :--- | :--- | :--- | :--- | :--- |
...
*\*Note: The maximum number of unique items in the stream is 1,000,000.*

![Phase 6 Tradeoff Graph](results/phase6_jl/phase6_jl_tradeoff.png)

### 7.2 Analysis of the Pareto Tradeoff
The results reveal a fascinating tradeoff between quantity (number of items retained) and quality (vector fidelity):

1. **JL Projection Dominates Tight Budgets:** At highly constrained memory budgets (1% to 10%), JL projection is overwhelmingly superior. At the 5% budget, the $d=64$ JL projection achieves an incredible **38.9%** recall compared to the baseline's **9.8%**. Storing 6x more lossy items provides drastically better semantic coverage than storing fewer perfect items.
2. **The "Distortion Ceiling":** As the memory budget increases to 25% and 50%, the recall for JL projected policies flatlines ($d=64$ hits a hard ceiling at 55.6%). Even though these policies have enough capacity to store every single unique item, the compression distortion alters spatial relationships so much that you cannot surpass this ceiling. At loose budgets (50%), the uncompressed 384D vectors easily win with 100% recall.

---

## 8. Final Conclusion

The **LSH Bucket-Occupancy Retention** policy is a high-performance, semantically-aware alternative to simple FIFO or Reservoir sampling for streaming memory. 

1. **In Redundancy-Heavy Streams**: It achieves perfect recall at half the memory budget where baselines plateau at ~69%.
2. **In Complex Conversations (LoCoMo)**: It significantly improves LLM-QA accuracy at medium-to-low memory budgets by preserving semantic diversity rather than chronological recency.
3. **With Dimension Reduction**: When memory is severely constrained (<15%), pairing BucketOccupancy with JL Projection ($d=64$) exponentially compounds its effectiveness, boosting recall by up to 400% by sacrificing a small amount of precision to store 6x more items.
4. **Computational Efficiency**: While 12x slower than Reservoir sampling, it maintains a throughput of >75k items/second, completely avoiding the $O(\|M\|)$ bottleneck of exact Semantic Deduplication.

**Recommendation**: Use BucketOccupancy in environments with high semantic repetition (e.g., sensor logs, chat history, repeated prompt contexts). If memory is severely constrained, pair it with a JL projection to maximize coverage.
